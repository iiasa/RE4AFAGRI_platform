...
for (i in 1:length(f_out)){
  ...
  names(f_out[[i]])[grep("residential_tt_2020", colnames(f_out[[1]])):(length(names(f_out[[i]]))-1)] <- paste0(names(f_out[[i]])[grep("residential_tt_2020", colnames(f_out[[1]])):(length(names(f_out[[i]]))-1)], "_", stri0)
...
for (i in c(2:length(f_out))){
  ...
  f_out[[i]] <- dplyr::select(f_out[[i]], grep("residential_tt_2020", colnames(f_out[[1]])):(length(names(f_out[[i]]))))
...
if(file.exists(paste0("C:/Users/falchetta/OneDrive - IIASA/IIASA_official_RE4AFAGRI_platform/mled/results/", ctr, "_gadm2_with_mled_loads_ALL_SCENARIOS_nourls.geojson"))){
...
f_mled <- f_mled[grepl(ctr, f_mled)]
#
...
#
  ...
  stri3 <- ifelse(i==1, "_s1", ifelse(i==2, "_s2", "_s3"))
#
  names(f_out_mled[[i]])[2:(length(names(f_out_mled[[i]]))-1)] <- paste0(names(f_out_mled[[i]])[2:(length(names(f_out_mled[[i]]))-1)], "_", stri0)
#
}
#
#
...
#
  f_out_mled[[i]] <- dplyr::select(f_out_mled[[i]], 2:(length(names(f_out_mled[[i]]))))
#
...
if (is.null(sf$NAME_2)){
  sf$NAME_2 <- sf$NAME_1
}

f_out_mled2 <- st_join(f_out_mled, (sf %>% dplyr::select(NAME_2)), st_intersects)
#
sf_split <- split(f_out_mled2, f_out_mled2$NAME_2)
#
up <- vector()
#
