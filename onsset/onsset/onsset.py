...
class SettlementProcessor:
    ...
    def estimating_non_residential_loads(self, year, time_step, start_year, productive_demand, commercial_demand=True, agri_demand=True):
        ...
        if int(productive_demand) == 1:
            ...
            if commercial_demand:
                ...
                self.df[SET_COMMERCIAL_DEMAND] = np.where(self.df[SET_POP_CALIB] > 100, (self.df['Commercial_Multiplier'] * self.df[SET_CAPITA_DEMAND] * self.df[SET_POP + "{}".format(str(start_year+2*time_step))]), 0)
            ...
            if agri_demand:
                ## Total estimated demand at the end year
                self.df[SET_AGRI_DEMAND] = np.where(self.df[SET_URBAN] == 0, (self.df[SET_CAPITA_DEMAND] * self.df[SET_POP + "{}".format(str(start_year+2*time_step))] * 0.1), 0)
                ## Demand in the specific year
    ...
    def apply_mled_demand(self, year):
        """this method applies the MLED calculated demands for each settlement per year
        ...
        """
        self.df[SET_ENERGY_PER_CELL + "{}".format(year)] = self.df[SET_CAPITA_DEMAND + "{}".format(year)] * self.df[SET_NEW_CONNECTIONS + "{}".format(year)]
        self.df[SET_TOTAL_ENERGY_PER_CELL] = self.df[SET_CAPITA_DEMAND + "{}".format(year)] * self.df[SET_POP + "{}".format(year)]
        self.df[SET_TOTAL_ENERGY_PER_CELL + "{}".format(year)] = self.df[SET_CAPITA_DEMAND + "{}".format(year)] * self.df[SET_POP + "{}".format(year)]
    ...
    def apply_limitations(self, eleclimit, year, time_step, prioritization, auto_densification=0):
        ...
        print("Total electrification rate achieved in {} is {:.1f} %".format(year, elecrate * 100))
