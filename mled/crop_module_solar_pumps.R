...
clusters_voronoi <- read_sf(paste0(input_country_specific,  paste0("clusters_voronoi_", countrystudy, ".gpkg")))
...
if (watercrop_unit =="mm"){
  rainfed <- pblapply(1:nlyr(files), function(X) {(rainfed[[X]] * (files[[X]] / crops_efficiency_irr$eta_irr[X]) * 10)})
} else{ 
  rainfed <- pblapply(1:nlyr(files), function(X) {(rainfed[[X]] / crops_efficiency_irr$eta_irr[X])})
}
...
if(field_size_contraint==T){field_size <- project(field_size, mask_raster_to_polygon(irrigated2[[1]], st_as_sfc(st_bbox(clusters_voronoi))), method = "bilinear") ; m <- field_size; m[m > 29] <- NA; field_size <- mask(field_size, m); irrigated2 <- pblapply(irrigated2, function(X){return(mask_raster_to_polygon(X, st_as_sfc(st_bbox(clusters_voronoi))))}); for (i in 1:length(irrigated2)){crs(irrigated2[[i]]) <- crs(field_size)}; field_size <- project(field_size,irrigated2[[1]]); irrigated2 <- pblapply(irrigated2, function(X){mask(X, field_size)})}
...
if (watercrop_unit =="mm"){
  irrigated <- pblapply(1:nlyr(files), function(X) {(irrigated[[X]] * (files[[X]] / crops_efficiency_irr$eta_irr[X]) * 10)})
} else{ 
  irrigated <- pblapply(1:nlyr(files), function(X) {(irrigated[[X]] / crops_efficiency_irr$eta_irr[X])})
}
...
clusters_voronoi_data <- dplyr::select(clusters_voronoi_data, id, starts_with("monthly") &  !contains("GQ"), starts_with("yearly") &  !contains("GQ"), area, maxflow)
