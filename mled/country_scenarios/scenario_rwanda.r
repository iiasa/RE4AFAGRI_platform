...
national_official_population = 12627000 # number of people
national_official_elrate = 0.495 # national residential electricity access rate
...
electr_final_demand_tot <-  1000000000 #https://www.google.com/search?q=rwanda+electricity+demand+by+sector&sxsrf=ALiCzsYBS6x_fP3ozOc_Y7NONm79cNnMWw:1660815820284&source=lnms&tbm=isch&sa=X&ved=2ahUKEwj-9oSIjdD5AhWlXfEDHQVYB94Q_AUoAXoECAEQAw&biw=1138&bih=526&dpr=2.81#imgrc=NURMTkit0erhTM

industry_final_demand_tot <- 400000000

residential_final_demand_tot <- 350000000

other_final_demand_tot <- electr_final_demand_tot - industry_final_demand_tot - residential_final_demand_tot

cropland_equipped_irrigation = 0.0392 #https://tableau.apps.fao.org/#/views/ReviewDashboard-v1/country_dashboard
...
clusters <- read_sf(find_it("clusters_Rwanda_GRID3_above5population.gpkg"), crs=4326)
...
clusters_nest <- gadm1 #read_sf(find_it("Rwanda_NEST_delineation.shp"))
...
crops = readxl::read_xlsx(find_it('crops_cfs_ndays_months_RWA.xlsx'))
...
health$Tier <- ifelse(health$amenity=="pharmacy" | is.na(health$amenity), 1, NA)
health$Tier <- ifelse(health$amenity=="health_post" | health$amenity=="dentist", 2, health$Tier)
health$Tier <- ifelse(health$amenity=="clinic", 3, health$Tier)
health$Tier <- ifelse(health$amenity=="hospital", 4, health$Tier)
...
#save.image(paste0(processed_folder, "scenario_rwanda.Rdata"))
